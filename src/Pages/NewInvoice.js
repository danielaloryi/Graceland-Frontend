...
const NewInvoice = () => {
    ...
    useEffect(() => {
        axios.get(`${BASEURL}/clients/${location?.state?.id}`).then((response) => {
            ...
            dispatch(Add_Bed({
                bedname: data?.tbldeceased?.bed
            }));

            dispatch(Add_Payment({
                payment: data?.tbldeceased?.payment_amount
            }))

            dispatch(Add_Deceased({
                fname: data?.tbldeceased?.fname,
                lname: data?.tbldeceased?.lname,
                dod: data?.tbldeceased?.dod,
                aged: data?.tbldeceased?.Aged,
                dobrial: data?.tbldeceased?.dobrial
            }));
